...
__per_packet_lb_svc_xlate_4(void *ctx, struct iphdr *ip4, __s8 *ext_err)
{
    ...
    if (svc && tuple.nexthdr == IPPROTO_UDP) {
        ...
        for (idx = 0; idx < MAX_DUP_BACKENDS; idx++) {
            ...
            if (dbv) {
                trace_printk(
                  "dup_backends lookup idx=%d ip=%pI4 ifidx=%u\n",
                  sizeof("dup_backends lookup idx=%d ip=%pI4 ifidx=%u\n"),
                  idx,
                  &dbv->ip,
                  dbv->ifindex
                );
                ...
                trace_printk(
                  "dup_backends lookup idx=%d → <nil>\n",
                  sizeof("dup_backends lookup idx=%d → <nil>\n"),
                  idx
                );
            }
            ...
            trace_printk(
              "dup_backends cloning idx=%d ifidx=%u\n",
